...
def picture_masked_cut_binary(img_file, threshold_for_circle, border_distance, threshold_binary, correction_radius, show_circle=False, show_image_switch=False) :
    ...
    diameter_corrected = y_max - y_zero

    print(diameter_corrected)
...
def check_image_for_black_and_border(img_file, contour_file, grid_size_in_pixel, show_image_switch=False):
    ...
    for j in range(num_grid_x):
        for i in range(num_grid_y):
            ...
            cur_image = img_file[i * grid_size_in_pixel:(i + 1) * grid_size_in_pixel,
            ...
            cur_image_mask = contour_file[i * grid_size_in_pixel:(i + 1) * grid_size_in_pixel,
                             j * grid_size_in_pixel:(j + 1) * grid_size_in_pixel]
    ...
    return check_array, num_grid_x, num_grid_y
...
if __name__ == "__main__":
    ...
    frame_threshold, mask_two = picture_masked_cut_binary(img_file, 170, 20, 140, 60,  show_circle=False, show_image_switch=True)

   # show_voxel_grid(300, frame_threshold)

    array = check_image_for_black_and_border(frame_threshold, mask_two, 300, show_image_switch=True)
